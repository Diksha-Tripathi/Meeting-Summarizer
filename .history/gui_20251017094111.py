...
class MeetingSummarizer(QMainWindow):
    ...
    def stop_recording(self):
        if self.process.state() == QProcess.Running:
            self.process.terminate() # A more graceful way to stop
    ...
    def handle_stdout(self):
        ...
        # Process buffer line-by-line looking for our JSON block
        if "SUMMARY_JSON_START" in self.output_buffer and "SUMMARY_JSON_END" in self.output_buffer:
            try:
             ...
             print(f"STDOUT: {data.strip()}") # Print other stdout for debugging
    ...
    def handle_stderr(self):
        ...
        # Display errors in a message box for better visibility
        if data:
